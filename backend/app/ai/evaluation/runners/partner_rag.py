"""Eval runner + checker for the ``partner_rag`` family.

Knowledge-base RAG **grounding + audience isolation + injection defense** for
the partner assistant, evaluated at real, deterministic seams — no DB, no
...
- ``app.ai.retrieval.citation_verify.verify_citations`` (§6.5) — the
  ...
  citation must name a document that was ACTUALLY retrieved this turn; an
  ungrounded citation is stripped + flagged as a hallucination risk. This is
  how "answer cites the doc / honest not-enough-info" is enforced offline.
- ``app.ai.safety.input_guard.neutralize_untrusted_text`` — a poisoned KB
  chunk (injection embedded in retrieved text) is defused before it re-enters
  the model context.
- the ``knowledge_base_query`` ToolSpec + the shared RBAC matrix
  (``partner_chat``) — the retrieval tool is ``read_only``, carries no
  org/audience-targeting parameter (a partner literally cannot ask for another
  org's internal docs), and is invisible to a guest.
...
def _run_rbac_seam(inp: dict[str, Any], data: dict[str, Any]) -> None:
    ...
    if not name:
        return
    visible = pc.visible_tool_names(name)
...
async def run_case(case: dict[str, Any]) -> Probe:
    ...
    return Probe(kind="partner_rag", blob=blob, data=data)
