...
CV/JD-specific structure, because a recruiter may attach any document or image.
Nothing the model returns is trusted verbatim: text is scrubbed and lengths are
capped, and a non-analyzable/blank/junk file yields a clean "couldn't analyse"
result — never a fabricated one. Provider/model/token/OCR internals never appear
in the output.
"""
...
You are given one or more images of a document a recruiter attached (and possibly
its raw embedded text). Transcribe and analyse ONLY what is visibly present.
    ...
    def analyze(
        ...
        *,
        ...
        try:
            ...
            log_ai_usage(
                task_type="attachment_vision_analysis",
                alias=alias,
                success=False,
...
def _detect_table(text: str) -> dict | None:
    ...
    for delim in ("\t", "|", ","):
        ...
        body = [
            [_clean_cell(c) for c in r[:_MAX_TABLE_COLS]] for r in shaped[1 : _MAX_TABLE_ROWS + 1]
        ]
        if not body:
            continue
...
def analyze_document(
    ...
    *,
    ...
    if not text_for_struct and (vision is None or not summary):
        ...
        return AttachmentAnalysis(status="not_analyzable", kind=kind_label, degraded=degraded)
