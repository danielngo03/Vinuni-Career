...
class AiTaskRunner:
    ...
    async def _record_billable(
        ...
        """Best-effort settlement through the energy choke point — NEVER breaks the call.

        No-op unless a ``UsageContext`` was supplied. Delegates to
        :func:`energy_service.charge`, the single settlement point: it writes the
        idempotent ``ai_billable_usage`` row (with the correct ``base_units`` from
        the per-feature weight table) AND, on a chargeable user-scoped result,
        decrements the student's masked energy account in the same transaction.

        Energy now derives ``base_units`` from the weight table keyed by
        ``ctx.feature_key`` — the runner's ``charge_units`` constructor arg is kept
        for partner call sites that still read it, but it no longer drives the
        student energy charge. The ledger facade savepoint-isolates its own insert;
        this extra guard swallows any residual error so accounting can never
        disrupt the user's response.
        """
