# Version: 2 | Date: 2026-07-06 | Author: ai-engineer
...
# Previous: v1 (initial version).
# Change from v1: the deterministic rules engine now feeds two AUTHORITATIVE
#       ground-truth blocks into the CONTEXT — ALREADY-SATISFIED REQUIREMENTS and
#       CONFIRMED GAPS. The model MUST treat ALREADY-SATISFIED items (which the
#       rules engine already resolved through synonyms/abbreviations, e.g.
#       "k8s" == "Kubernetes") as fully met and MUST NOT list them as gaps or
#       suggest the candidate add them. This stops the explanation layer from
#       contradicting the deterministic evidence (e.g. telling a candidate to
#       "add Kubernetes" when their CV already writes "k8s").
"""Prompt template for ``cv_fit_analysis`` semantic scorer (v2).
