"""Chat-attachment upload + AI data-analysis service (university control-plane).
...
- :func:`analyze_attachment` — load the caller's OWN attachment (owner + org
  scoped), run the cost-tiered analysis cascade OFFLOADED off the event loop, turn
  it into a structured, leakage-safe DATA ANALYSIS (``tables`` + render-ready
  ``charts`` + ``insights`` + ``summary``), cache the result on ``analysis_json``,
  and meter the energy ledger exactly once on a genuine result. Re-analysing a
  stored result (or re-uploading identical bytes) returns the cache without
  re-charging (idempotent, keyed by the content hash).

This mirrors the partner-ai-overhaul worktree's ``attachment_service`` (same
function names + signatures + owner/org scoping) so the merge dedups; the only
university-specific difference is the richer ``tables``/``charts``/``insights``
result shape (partner returns a single ``table`` + ``key_values``) and the
tier-aware energy weight (the vision tier costs more). Keep the superset at merge.

RBAC: the endpoint requires session ownership; the tool additionally passes the
central ``dispatch.authorize_tool`` gate (persona family + ``required_permissions``)
before this service runs. Both paths only ever touch the caller's own attachment —
a cross-user / cross-org attachment id is indistinguishable from a missing one
(404), matching the tenant-isolation convention.
"""
...
from app.ai.energy.constants import (
    FEATURE_ATTACHMENT_ANALYSIS,
    FEATURE_ATTACHMENT_VISION_ANALYSIS,
)
from app.ai.energy.service import (
    build_usage_context,
    charge_units,
    resolve_primary_department_id,
)
from app.ai.extraction import attachment as analysis
from app.ai.extraction import cv_validation, data_analysis
...
_ALLOWED_EXTS = frozenset({".pdf", ".png", ".jpg", ".jpeg", ".docx", ".txt", ".csv"})
...
async def _load_owned(
    ...
    # A staff/partner attachment is org-scoped; a cross-org id is indistinguishable
    # from missing. For a principal without an org this is a plain owner check.
...
def _to_public(outcome: analysis.AttachmentAnalysis, derived: dict) -> dict:
    """The leakage-safe university analysis result the assistant renders.

    Superset of the extraction outcome + the deterministic data-analysis layer:
    tables + render-ready charts + factual insights + the neutral summary. No
    storage key, provider/model/token/OCR internals, or another user's data.
    """
    ...
    return {
        ...
        "tables": derived.get("tables", []),
        "charts": derived.get("charts", []),
        "extracted_text_preview": outcome.extracted_text_preview,
    }
...
async def analyze_attachment(
    ...
    *,
    ...
    """Analyse the caller's own attachment into tables/charts/insights; cache + meter.

    Returns the leakage-safe analysis dict (``cached`` flags a stored result).
    Charges the attachment-analysis feature exactly once per genuine analysis
    (idempotent on the CONTENT hash, so re-uploading identical bytes never
    double-charges); a re-analysis of a stored result returns the cache without
    charging, and a rejected/blank/unsupported file is never charged. The vision
    tier debits the higher weight.
    """
    ...
    derived = data_analysis.build_data_analysis(outcome)
    public = _to_public(outcome, derived)
    ...
    if outcome.chargeable:
        ...
        await _meter(
            ...
            principal=principal,
            row=row,
            vision_used=outcome.vision_used,
...
async def _meter(
    ...
    *,
    ...
    """Debit one attachment-analysis charge (best-effort, idempotent).

    Scope resolves to the partner ORG pool for a recruiter, the user scope
    otherwise, with the acting member's primary department attributed for
    per-department university accounting. The vision tier debits the higher
    weight. Idempotent on the CONTENT hash so a retry / identical re-upload never
    double-charges. Accounting must never break the tool result, so failures are
    swallowed.
    """

    feature_key = (
        FEATURE_ATTACHMENT_VISION_ANALYSIS if vision_used else FEATURE_ATTACHMENT_ANALYSIS
    )
    try:
        ...
        ctx = build_usage_context(
            ...
            idempotency_parts=(content_hash,),
        )
        await record_billable_usage(
            session,
            ctx=ctx,
            result_status=RESULT_SUCCESS,
            base_units=charge_units(feature_key),
        )
