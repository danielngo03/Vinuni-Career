...
- :func:`analyze_attachment` — load the caller's OWN attachment (owner + org
  ...
  and cache the structured, leakage-safe result on ``analysis_json``.
  Re-analysing a stored result returns the cache (idempotent); the vision tier
  logs its own ``ai_usage_log`` cost entry when a model is actually invoked.
...
async def _load_owned(
    ...
    row = (await session.execute(select(ChatAttachment).where(*conds))).scalar_one_or_none()
    if row is None:
...
async def analyze_attachment(
    ...
    *,
    ...
    if outcome.chargeable:
        # Cost is already tracked by the vision tier's own ai_usage_log entry
        # (when a model is actually invoked); the enclosing chat turn is
        # request-quota gated. This checkout has no separate energy ledger.
