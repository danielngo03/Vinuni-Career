...
from app.ai.gateway import ChatMessage, ChatRequest, RerankRequest, get_llm_gateway
from app.ai.gateway.errors import LLMGatewayError
from app.ai.gateway.schemas import Role
from app.ai.retrieval import index_text
from app.ai.safety import detect_prompt_injection
from app.modules.ai_operations.api.legacy_schemas import MatchResponse
from app.platform.cache import get_cache
from app.platform.database.models import AIUsageLog
...
def chat_with_gateway(
    ...
    *,
    ...
    request = ChatRequest(
        messages=[
            ChatMessage(role=cast(Role, message["role"]), content=message["content"])
...
def rerank_with_gateway(
    query: str,
    documents: list[str],
    top_n: int,
    model: str | None = None,
) -> dict[str, Any]:
    request = RerankRequest(query=query, documents=documents, top_n=top_n, model=model)
    response = get_llm_gateway().rerank(request)
    return {
        "results": [
            {
                "index": item.index,
...
def log_ai_usage(
    ...
    *,
    ...
    if raw_usage:
        input_tokens = raw_usage.get("prompt_tokens", input_tokens)
        output_tokens = raw_usage.get("completion_tokens", output_tokens)
    ...
    if hasattr(usage, "provider"):
        usage.provider = provider
    if hasattr(usage, "model"):
        usage.model = model
    if hasattr(usage, "request_id"):
...
def provider_status() -> dict[str, list[str]]:
    return {
        ...
        "embedding_chain": settings.embedding_provider_chain or ["offline"],
