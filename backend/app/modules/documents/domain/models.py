...
class CvProfile(Base):
    ...
    # INTERNAL matching snapshot stored when a CV is COMMITTED to the library
    # (finalize / upload-import) — a version-stamped, LOSSLESS projection of the
    # sections CV-JD scoring consumes (``{content_version, sections, last_activity_at,
    # schema_version}``); see ``cv_lifecycle_service.build_matching_representation``.
    # Deterministic, no OCR/AI. NULL for drafts. Consumed by
    # ``job_fit_service._build_cv_input`` as a fast path while fresh (its
    # ``content_version`` still equals ``version``); being byte-identical to a live
    # section load it never changes the score. NEVER surfaced in a student-facing
    # response; presenters must not include it.
