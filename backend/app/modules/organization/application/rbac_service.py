...
async def _role_permission_strings(session: AsyncSession, role_id: uuid.UUID) -> list[str]:
    rows = (
        await session.execute(
            select(Permission.resource_type, Permission.action).where(Permission.role_id == role_id)
        )
    ).all()
...
async def list_roles(
    ...
    roles = (
        (await session.execute(select(Role).where(Role.org_id == org_id).order_by(Role.created_at)))
        .scalars()
        .all()
    )
    out = []
...
async def create_role(
    ...
    *,
    ...
    existing = (
        await session.execute(select(Role.id).where(Role.org_id == org_id, Role.name == name))
    ).first()
...
async def update_role(
    ...
    *,
    ...
    if name is not None and name != role.name:
        ...
        clash = (
            await session.execute(
                select(Role.id).where(Role.org_id == org_id, Role.name == name, Role.id != role.id)
            )
        ).first()
    ...
    if permissions is not None:
        ...
        will_have_admin = (
            catalog.ADMIN_WILDCARD_RESOURCE,
            catalog.ADMIN_WILDCARD_ACTION,
        ) in new_set
        ...
        await session.execute(delete(Permission).where(Permission.role_id == role.id))
...
async def _get_department(
    ...
    stmt = select(Department).where(Department.id == dept_id, Department.org_id == org_id)
    return (await session.execute(stmt)).scalar_one_or_none()


async def list_departments(session: AsyncSession, *, principal: Principal) -> list[dict]:
    org_id = _require_org(principal)
...
async def create_department(
    ...
    *,
    ...
    org_id = _require_org(principal)
    permission_checker.require(principal, "departments", "create", resource_org_id=org_id)
    ...
    clash = (
        await session.execute(
            select(Department.id).where(Department.org_id == org_id, Department.name == name)
        )
    ).first()
    if clash is not None:
        raise DuplicateNameError()
...
async def update_department(
    ...
    *,
    ...
    org_id = _require_org(principal)
    permission_checker.require(principal, "departments", "update", resource_org_id=org_id)
...
async def delete_department(
    ...
    *,
    ...
    org_id = _require_org(principal)
    permission_checker.require(principal, "departments", "delete", resource_org_id=org_id)
    dept = await _get_department(session, org_id=org_id, dept_id=dept_id)
    if dept is None:
        raise ResourceNotFoundError()
    # membership_departments links cascade via FK; child departments are detached.
    await session.execute(
        update(Department).where(Department.parent_id == dept.id).values(parent_id=None)
    )
