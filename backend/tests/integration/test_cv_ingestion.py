...
async def test_import_applies_review_field_overrides(db_session) -> None:
    # The student edits a low-confidence skill field on the review screen; the
    # edited value (not the original extracted value) must land in the draft.
    await template_seed.ensure_default_templates(db_session)
    await db_session.commit()
    _u, student = await make_student(db_session)
    ing = await _ingest_ready(db_session, student)
    ...
    paths = {f["path"] for f in ing["review_fields"]}
    assert "skills[0].text" in paths
    original_skill = next(
        f["value"] for f in ing["review_fields"] if f["path"] == "skills[0].text"
    )
    edited_skill = "Go, Rust, Kubernetes, Terraform"
    assert edited_skill != original_skill

    detail = await ingestion_service.import_ingestion(
        db_session, principal=student, ingestion_id=uuid.UUID(ing["ingestion_id"]),
        payload={
            "title": "Imported CV",
            "overrides": [{"path": "skills[0].text", "value": edited_skill}],
        },
        ctx=CTX,
    )
    # The edited value is in the created draft's skills section...
    assert edited_skill in _section_texts(detail, "skills")
    ...
    stored_skill = ing_row.extracted_data["skills"]["items"][0]["text"]
    assert stored_skill == original_skill
    ...
    detail = await ingestion_service.import_ingestion(
        ...
        payload={
            ...
            "overrides": [
                ...
                {"path": "skills[99].text", "value": "out of range"},
            ],
        },
        ctx=CTX,
    )
    # No forbidden section type appears; skills untouched by the rejected paths.
    ...
    with pytest.raises(ValidationFailedError):
        await ingestion_service.import_ingestion(
            ...
            payload={
                ...
                "overrides": [{"path": "skills[0].text", "value": "edited"}],
            },
            ctx=CTX,
        )
...
async def test_import_with_overrides_respects_quota(db_session) -> None:
    ...
    with pytest.raises(CvQuotaReachedError) as exc:
        await ingestion_service.import_ingestion(
            db_session, principal=student, ingestion_id=uuid.UUID(ing["ingestion_id"]),
            payload={"overrides": [{"path": "skills[0].text", "value": "edited"}]},
            ctx=CTX,
        )
...
async def test_import_override_reject_excludes_item(db_session) -> None:
    ...
    detail = await ingestion_service.import_ingestion(
        ...
        payload={
            "title": "Imported CV",
            "overrides": [{"path": "skills[0].text", "accepted": False}],
        },
        ctx=CTX,
    )
    assert _section_texts(detail, "skills") == []
    assert original_skill not in json.dumps(detail)
