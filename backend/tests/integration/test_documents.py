...
async def test_upload_happy_creates_review_run_with_fields(db_session) -> None:
    ...
    # Free-text sections (Objective -> summary) keep per-field review markers;
    # career sections (education/experience) are now structured entries (B-599).
    assert any(f["path"].startswith("summary") for f in run["review_fields"])
...
async def test_finalize_writes_matching_representation(db_session) -> None:
    ...
    # The snapshot is version-stamped with the COMMITTED (post-finalize) version so
    # a later section edit invalidates it (fast path falls back to live sections).
    assert matching["content_version"] == detail["version"]
    assert matching["last_activity_at"]
    # It is a LOSSLESS {section_type,title,content} projection of the CV's sections
    # (the exact shape the deterministic scorer consumes) — one entry per section.
    by_type = {s["section_type"]: s for s in matching["sections"]}
    assert {"header", "skills", "experience"} <= set(by_type)
    assert by_type["skills"]["content"]["items"][0]["name"] == "Python"
    assert by_type["skills"]["content"]["items"][0]["level"] == 90
    assert by_type["experience"]["content"]["entries"][0]["heading"] == "Software Intern"
    assert by_type["header"]["content"]["name"] == "Nguyen Van A"
