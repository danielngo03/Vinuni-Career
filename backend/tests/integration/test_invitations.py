...
async def test_send_invite_duplicate_raises_conflict(db_session) -> None:
    ...
    await invitation_service.send_invite(
        db_session, principal=partner, job_id=job_id, student_id=student.user_id,
    )
...
async def test_send_invite_requires_partner(db_session) -> None:
    ...
    with pytest.raises(PermissionDeniedError):
        await invitation_service.send_invite(
            db_session, principal=student, job_id=job_id, student_id=student.user_id,
        )
...
async def test_respond_twice_raises_conflict(db_session) -> None:
    ...
    await invitation_service.respond_invitation(
        db_session, principal=student, invitation_id=inv["id"], response="declined",
    )
    with pytest.raises(ConflictError):
        await invitation_service.respond_invitation(
            db_session, principal=student, invitation_id=inv["id"], response="accepted",
        )
...
async def test_respond_wrong_student_raises_not_found(db_session) -> None:
    ...
    with pytest.raises(ResourceNotFoundError):
        await invitation_service.respond_invitation(
            db_session, principal=student2, invitation_id=inv["id"], response="accepted",
        )
