...
async def test_profile_quality_rubric_is_deterministic_and_explainable(db_session) -> None:
    ...
    data = await crm_service.get_profile_quality(db_session, principal=admin, org_id=org.id)
...
async def test_profile_quality_denied_for_other_partner_org(db_session) -> None:
    ...
    with pytest.raises(PermissionDeniedError):
        await crm_service.get_profile_quality(db_session, principal=admin_a, org_id=org_b.id)
...
async def test_profile_quality_visible_to_university_actor(db_session) -> None:
    ...
    data = await crm_service.get_profile_quality(db_session, principal=uni_admin, org_id=org.id)
    assert 0 <= data["score"] <= 100
...
async def test_recruiter_seats_reflects_active_members_and_cap(db_session) -> None:
    ...
    data = await crm_service.recruiter_seats_summary(db_session, principal=admin, org_id=org.id)
    assert data["used"] == 2
...
async def test_university_actor_can_set_and_audit_campus_owner(db_session) -> None:
    ...
    fetched = await crm_service.get_campus_owner(db_session, principal=uni_admin, org_id=org.id)
...
async def test_university_actor_raises_and_resolves_risk_flag(db_session) -> None:
    ...
    flag = await crm_service.raise_risk_flag(
        ...
        principal=uni_admin,
        org_id=org.id,
        flag_type="unverified_tax_id",
        severity="high",
        note="Tax ID pending verification.",
        ctx=CTX,
    )
    assert flag["is_resolved"] is False

    flags = await crm_service.list_risk_flags(db_session, principal=uni_admin, org_id=org.id)
    assert len(flags) == 1
    ...
    resolved = await crm_service.resolve_risk_flag(
        ...
        resolution_note="Verified via tax authority API.",
        ctx=CTX,
    )
...
async def test_university_actor_creates_note_and_audits(db_session) -> None:
    ...
    note = await crm_service.create_note(
        ...
        principal=uni_admin,
        org_id=org.id,
        body="Flaky invoice history, follow up next quarter.",
        ctx=CTX,
    )
...
async def test_crm_activity_rollup_empty_org(db_session) -> None:
    _admin_user, org, admin = await make_org_with_admin(db_session)
    data = await crm_service.event_campaign_rollup(db_session, principal=admin, org_id=org.id)
