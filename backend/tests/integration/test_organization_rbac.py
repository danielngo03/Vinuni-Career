...
async def test_member_without_permission_denied_403(db_session) -> None:
    ...
    _u, _m, principal = await add_member(db_session, org=org, permissions=[("members", "read")])
...
async def test_admin_can_create_role_and_audits(db_session) -> None:
    ...
    role = await rbac_service.create_role(
        ...
        name="Recruiter",
        description="hires",
        permissions=[("members", "read"), ("jobs", "read")],
        ctx=CTX,
    )
...
async def test_cross_tenant_role_read_returns_404(db_session) -> None:
    _u_a, org_a, admin_a = await make_org_with_admin(db_session, display_name="Org A")
    _u_b, org_b, admin_b = await make_org_with_admin(db_session, display_name="Org B")
    ...
    with pytest.raises(ResourceNotFoundError):
        await rbac_service.get_role(db_session, principal=admin_a, role_id=uuid.UUID(role_b["id"]))
...
async def test_member_role_assignment_respects_ceiling(db_session) -> None:
    ...
    strong = await rbac_service.create_role(
        ...
        name="Strong",
        description=None,
        permissions=[("members", "remove"), ("members", "update")],
        ctx=CTX,
    )
...
async def test_cannot_delete_last_admin_role(db_session) -> None:
    ...
    with pytest.raises(SystemRoleImmutableError):
        await rbac_service.delete_role(db_session, principal=admin, role_id=admin_role.id, ctx=CTX)
...
async def test_second_admin_lets_first_be_removed(db_session) -> None:
    ...
    refreshed = (
        await db_session.execute(select(Membership).where(Membership.id == m1.id))
    ).scalar_one()
...
async def test_membership_version_conflict(db_session) -> None:
    _admin_user, org, admin = await make_org_with_admin(db_session)
    _u, membership, _p = await add_member(db_session, org=org, permissions=[("members", "read")])
