...
async def test_advance_from_no_active_row_lands_at_stage_one(db_session) -> None:
    ...
    assert [r.status for r in rows2] == [pipeline.STAGE_PASSED, pipeline.STAGE_ACTIVE]
...
def test_rollback_schema_enforces_min_reason() -> None:
    ...
    ok = RollbackRequestBody(target_stage_id=uuid.uuid4(), reason=_GOOD_REASON, version=2)
    assert ok.version == 2
