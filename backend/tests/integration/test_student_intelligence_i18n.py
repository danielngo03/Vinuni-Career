...
# Stable localized marker phrases from `improvement_actions` / competition guidance
# (NOT learning-gap suggestions, which are skill-specific per B-588's catalog).
_VI_MARKERS = ("Bổ sung bằng chứng", "Hạn nộp hồ sơ")
_EN_MARKERS = ("Add evidence for", "deadline")
...
async def test_vi_vs_en_localized_guidance_differs(db_session) -> None:
    ...
    # A stable localized improvement-action phrase appears in each locale. Learning
    # -gap suggestions are skill-specific (B-588 catalog); their vi/en divergence is
    # asserted structurally below rather than by a fixed phrase.
...
async def test_default_locale_is_vietnamese(db_session) -> None:
    ...
    # proof is the improvement-action phrase above. Here just assert one is present.
    assert out["learning_gaps"][0]["suggestion"]
