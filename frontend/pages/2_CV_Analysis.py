...
from backend.src.models.schemas import student_from_dict, student_to_dict
...
tab_parse, tab_manage = st.tabs(["Parse CV", "Manage Students"])
...
with tab_parse:
    ...
    if "draft_student" in st.session_state:
        ...
        edited = st.text_area(
            "Validated student JSON",
            value=json.dumps(st.session_state["draft_student"], indent=2, ensure_ascii=False),
            height=420,
        )
        if st.button("Validate and save student"):
            try:
                student_data = json.loads(edited)
