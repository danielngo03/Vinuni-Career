...
// so the assistant (partner LLM path) sees the attachment id and calls its
// `analyze_attachment` tool. `SendMessageRequest` is text-only, so this is the
// only channel. The id is the user's own attachment id — not sensitive. The
// message-bubble renderer strips these lines back out and shows a paperclip
// chip instead, so the raw ref never appears in the visible bubble.
 ...
 * filenames referenced by any appended ref lines. Assistant text is never
 * parsed (only the user embeds refs), so callers pass user content only.
 */
